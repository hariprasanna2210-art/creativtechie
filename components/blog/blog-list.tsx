...
export function BlogList() {
  ...
  return (
    <>
      <div className="mb-10 grid gap-4 rounded-lg bg-white p-4 md:grid-cols-[1fr_auto]">
        <label className="flex items-center gap-3 rounded-md bg-[#edf5f8] px-5">
        ...
        <div className="flex flex-wrap gap-2">
          {cats.map((cat) => <button key={cat} onClick={() => setCategory(cat)} className={`rounded-md px-5 py-3 text-sm font-bold ${category === cat ? "bg-[#06345f] text-white" : "bg-[#edf5f8]"}`}>{cat}</button>)}
        </div>
      </div>
      <div className="grid gap-5 md:grid-cols-3">
        {posts.map((post, index) => (
          <article key={post.title} className={`${index === 0 ? "md:col-span-2" : ""} overflow-hidden rounded-xl bg-white`}>
            ...
            <div className="p-7">
              ...
              <p className="mt-4 leading-7 text-[#52616f]">A practical studio note written for founders, marketers and operators who want sharper digital decisions.</p>
            </div>
          </article>
        ))}
      </div>
